...
type PyTorchController struct {
	...
	updateStatusHandler func(job *v1beta2.PyTorchJob) error

	// To allow injection of deletePyTorchJob for testing.
	deletePyTorchJobHandler func(job *v1beta2.PyTorchJob) error
...
func NewPyTorchController(
	...
	jc := jobcontroller.NewJobController(pc, metav1.Duration{Duration: 15 * time.Second},
		option.EnableGangScheduling, kubeClientSet, kubeBatchClientSet, kubeInformerFactory, v1beta2.Plural)
	pc.JobController = jc
	// Set sync handler.
...
// manager.
func (pc *PyTorchController) satisfiedExpectations(job *v1beta2.PyTorchJob) bool {
	satisfied := false
	jobKey, err := KeyFunc(job)
	if err != nil {
		utilruntime.HandleError(fmt.Errorf("couldn't get key for job object %#v: %v", job, err))
...
func (pc *PyTorchController) GetAPIGroupVersionKind() schema.GroupVersionKind {
	return v1beta2.SchemeGroupVersionKind
}

func (pc *PyTorchController) GetAPIGroupVersion() schema.GroupVersion {
	return v1beta2.SchemeGroupVersion
}
