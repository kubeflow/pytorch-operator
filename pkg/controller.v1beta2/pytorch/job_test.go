...
func TestAddPyTorchJob(t *testing.T) {
	...
	ctr.deletePyTorchJobHandler = func(job *v1beta2.PyTorchJob) error {
		return nil
	}
...
func TestCopyLabelsAndAnnotation(t *testing.T) {
	...
	job.Spec.PyTorchReplicaSpecs[v1beta2.PyTorchReplicaTypeMaster].Template.Labels = labels
	job.Spec.PyTorchReplicaSpecs[v1beta2.PyTorchReplicaTypeMaster].Template.Annotations = annotations
	unstructured, err := testutil.ConvertPyTorchJobToUnstructured(job)
	if err != nil {
		t.Errorf("Failed to convert the PyTorchJob to Unstructured: %v", err)
	}
