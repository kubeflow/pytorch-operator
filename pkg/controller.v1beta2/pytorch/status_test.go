...
func TestFailed(t *testing.T) {
	...
	initializePyTorchReplicaStatuses(job, v1beta2.PyTorchReplicaTypeWorker)
	pod := testutil.NewBasePod("pod", job, t)
	pod.Status.Phase = v1.PodFailed
	updatePyTorchJobReplicaStatuses(job, v1beta2.PyTorchReplicaTypeWorker, pod)
	if job.Status.ReplicaStatuses[common.ReplicaType(v1beta2.PyTorchReplicaTypeWorker)].Failed != 1 {
		t.Errorf("Failed to set the failed to 1")
	}
	err := ctr.updateStatusSingle(job, v1beta2.PyTorchReplicaTypeWorker, 3, false)
	if err != nil {
...
func TestStatus(t *testing.T) {
	...
	for i, c := range testCases {
		...
		initializePyTorchReplicaStatuses(c.job, v1beta2.PyTorchReplicaTypeWorker)
		initializePyTorchReplicaStatuses(c.job, v1beta2.PyTorchReplicaTypeMaster)

		setStatusForTest(c.job, v1beta2.PyTorchReplicaTypeMaster, c.expectedFailedMaster, c.expectedSucceededMaster, c.expectedActiveMaster, t)
		setStatusForTest(c.job, v1beta2.PyTorchReplicaTypeWorker, c.expectedFailedWorker, c.expectedSucceededWorker, c.expectedActiveWorker, t)

		if _, ok := c.job.Spec.PyTorchReplicaSpecs[v1beta2.PyTorchReplicaTypeMaster]; ok {
			err := ctr.updateStatusSingle(c.job, v1beta2.PyTorchReplicaTypeMaster, 1, c.restart)
			if err != nil {
				t.Errorf("%s: Expected error %v to be nil", c.description, err)
			}
			if c.job.Spec.PyTorchReplicaSpecs[v1beta2.PyTorchReplicaTypeWorker] != nil {
				replicas := c.job.Spec.PyTorchReplicaSpecs[v1beta2.PyTorchReplicaTypeWorker].Replicas
				err := ctr.updateStatusSingle(c.job, v1beta2.PyTorchReplicaTypeWorker, int(*replicas), c.restart)
				if err != nil {
					t.Errorf("%s: Expected error %v to be nil", c.description, err)
				}
			}
...
func setStatusForTest(job *v1beta2.PyTorchJob, typ v1beta2.PyTorchReplicaType, failed, succeeded, active int32, t *testing.T) {
	pod := testutil.NewBasePod("pod", job, t)
	var i int32
	for i = 0; i < failed; i++ {
		pod.Status.Phase = v1.PodFailed
