...
func TestClusterSpec(t *testing.T) {
	...
	testCase := []tc{
		tc{
			...
			rt:                  v1beta2.PyTorchReplicaTypeMaster,
			index:               "0",
			totalPods:           1,
		...
		tc{
			job:                 testutil.NewPyTorchJobWithMaster(1),
			rt:                  v1beta2.PyTorchReplicaTypeWorker,
			index:               "0",
			totalPods:           2,
...
func TestRestartPolicy(t *testing.T) {
	type tc struct {
		...
		expectedType          v1beta2.PyTorchReplicaType
	}
	testCase := []tc{
		func() tc {
			job := testutil.NewPyTorchJobWithMaster(1)
			specRestartPolicy := common.RestartPolicyExitCode
			job.Spec.PyTorchReplicaSpecs[v1beta2.PyTorchReplicaTypeMaster].RestartPolicy = specRestartPolicy
			return tc{
				job: job,
				expectedRestartPolicy: v1.RestartPolicyNever,
				expectedType:          v1beta2.PyTorchReplicaTypeMaster,
			}
		}(),
		func() tc {
			job := testutil.NewPyTorchJobWithMaster(1)
...
func TestExitCode(t *testing.T) {
	...
	job := testutil.NewPyTorchJobWithMaster(1)
	job.Spec.PyTorchReplicaSpecs[v1beta2.PyTorchReplicaTypeMaster].RestartPolicy = common.RestartPolicyExitCode
	unstructured, err := testutil.ConvertPyTorchJobToUnstructured(job)
	if err != nil {
