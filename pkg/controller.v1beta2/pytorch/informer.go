...
import (
	...
	"github.com/kubeflow/pytorch-operator/pkg/common/util/v1beta2/unstructured"
...
func NewUnstructuredPyTorchJobInformer(restConfig *restclientset.Config, namespace string) jobinformersv1beta2.PyTorchJobInformer {
	...
	resource := schema.GroupVersionResource{
		Group:    v1beta2.GroupName,
		Version:  v1beta2.GroupVersion,
		Resource: v1beta2.Plural,
	}
...
func (pc *PyTorchController) NewPyTorchJobInformer(jobInformerFactory jobinformers.SharedInformerFactory) jobinformersv1beta2.PyTorchJobInformer {
	return jobInformerFactory.Kubeflow().V1beta2().PyTorchJobs()
}

func (pc *PyTorchController) getPyTorchJobFromName(namespace, name string) (*v1beta2.PyTorchJob, error) {
	key := fmt.Sprintf("%s/%s", namespace, name)
	return pc.getPyTorchJobFromKey(key)
}

func (pc *PyTorchController) getPyTorchJobFromKey(key string) (*v1beta2.PyTorchJob, error) {
...
func jobFromUnstructured(obj interface{}) (*v1beta2.PyTorchJob, error) {
	...
	var job v1beta2.PyTorchJob
	err := runtime.DefaultUnstructuredConverter.FromUnstructured(un.Object, &job)
	logger := pylogger.LoggerForUnstructured(un, v1beta2.Kind)
	if err != nil {
		logger.Errorf(failedMarshalMsg, err)
		return nil, errFailedMarshal
	}

	err = validation.ValidateBetaTwoPyTorchJobSpec(&job.Spec)
	if err != nil {
		logger.Errorf(failedMarshalMsg, err)
		return nil, errFailedMarshal
	}
...
func unstructuredFromPyTorchJob(obj interface{}, job *v1beta2.PyTorchJob) error {
	un, ok := obj.(*metav1unstructured.Unstructured)
