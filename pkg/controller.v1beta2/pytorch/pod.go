...
func (pc *PyTorchController) reconcilePods(
	...
	pods []*v1.Pod,
	rtype v1beta2.PyTorchReplicaType,
	...
	for index, podSlice := range podSlices {
		...
		if len(podSlice) > 1 {
			...
			if rtype == v1beta2.PyTorchReplicaTypeMaster {
				masterRole = true
			}
			...
			if spec.RestartPolicy == common.RestartPolicyExitCode {
				...
				for _, status := range pod.Status.ContainerStatuses {
					...
					if status.Name == v1beta2.DefaultContainerName && state.Terminated != nil {
...
func (pc *PyTorchController) createNewPod(job *v1beta2.PyTorchJob, rtype v1beta2.PyTorchReplicaType, index string, spec *common.ReplicaSpec, masterRole bool) error {
	rt := strings.ToLower(string(rtype))
...
func setClusterSpec(podTemplateSpec *v1.PodTemplateSpec, job *v1beta2.PyTorchJob, totalReplicas int32, index string, rtype v1beta2.PyTorchReplicaType) error {
	rank, err := strconv.Atoi(index)
	if err != nil {
		return err
	}

	masterPort, err := GetPortFromPyTorchJob(job, v1beta2.PyTorchReplicaTypeMaster)
	if err != nil {
		return err
	}

	masterAddr := jobcontroller.GenGeneralName(job.Name, strings.ToLower(string(v1beta2.PyTorchReplicaTypeMaster)), strconv.Itoa(0))
	if rtype == v1beta2.PyTorchReplicaTypeMaster {
		if rank != 0 {
