...
func (pc *PyTorchController) updatePyTorchJobStatus(job *v1beta2.PyTorchJob) error {
	_, err := pc.jobClientSet.KubeflowV1beta2().PyTorchJobs(job.Namespace).UpdateStatus(job)
	return err
}

// updatePyTorchJobConditions updates the conditions of the given job.
func updatePyTorchJobConditions(job *v1beta2.PyTorchJob, conditionType common.JobConditionType, reason, message string) error {
...
func initializePyTorchReplicaStatuses(job *v1beta2.PyTorchJob, rtype v1beta2.PyTorchReplicaType) {
	commonType := common.ReplicaType(rtype)
...
func updatePyTorchJobReplicaStatuses(job *v1beta2.PyTorchJob, rtype v1beta2.PyTorchReplicaType, pod *v1.Pod) {
	commonType := common.ReplicaType(rtype)
