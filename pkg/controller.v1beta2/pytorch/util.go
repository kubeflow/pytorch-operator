...
func GetPortFromPyTorchJob(job *v1beta2.PyTorchJob, rtype v1beta2.PyTorchReplicaType) (int32, error) {
	...
	for _, container := range containers {
		if container.Name == v1beta2.DefaultContainerName {
			ports := container.Ports
			for _, port := range ports {
				if port.Name == v1beta2.DefaultPortName {
...
func ContainMasterSpec(job *v1beta2.PyTorchJob) bool {
	if _, ok := job.Spec.PyTorchReplicaSpecs[v1beta2.PyTorchReplicaTypeMaster]; ok {
		return true
	}
	return false
}
