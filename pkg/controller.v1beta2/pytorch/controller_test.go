...
func TestNormalPath(t *testing.T) {
	testCases := map[string]struct {
		...
		"Distributed PyTorchJob (4 workers, 1 master) is created and all replicas are pending": {
			4, 1,
			nil, true,
			4, 0, 0, 0,
			1, 0, 0, 0,
		...
		"Distributed PyTorchJob (4 workers, 1 master) is created, 2 workers, 1 master are pending": {
			4, 1,
			nil, true,
			2, 0, 0, 0,
			1, 0, 0, 0,
		...
		"Distributed PyTorchJob (4 workers, 1 master) is created, 2 workers, 1 master are pending, 1 worker is running": {
			4, 1,
			nil, true,
			2, 1, 0, 0,
			1, 0, 0, 0,
		...
		"Distributed PyTorchJob (4 workers, 1 master) is created, 2 workers, 1 master are pending, 1 worker is succeeded": {
			4, 1,
			nil, true,
			2, 0, 1, 0,
			1, 0, 0, 0,
	...
	for name, tc := range testCases {
		...
		for _, controllerRef := range fakePodControl.ControllerRefs {
			if got, want := controllerRef.APIVersion, v1beta2.SchemeGroupVersion.String(); got != want {
				t.Errorf("controllerRef.APIVersion = %q, want %q", got, want)
			}
			if got, want := controllerRef.Kind, v1beta2.Kind; got != want {
		...
		if actual.Status.ReplicaStatuses[common.ReplicaType(v1beta2.PyTorchReplicaTypeWorker)] != nil {
			if actual.Status.ReplicaStatuses[common.ReplicaType(v1beta2.PyTorchReplicaTypeWorker)].Active != tc.expectedActiveWorkerPods {
				t.Errorf("%s: unexpected number of active pods.  Expected %d, saw %d\n", name, tc.expectedActiveWorkerPods, actual.Status.ReplicaStatuses[common.ReplicaType(v1beta2.PyTorchReplicaTypeWorker)].Active)
			}
			if actual.Status.ReplicaStatuses[common.ReplicaType(v1beta2.PyTorchReplicaTypeWorker)].Succeeded != tc.expectedSucceededWorkerPods {
				t.Errorf("%s: unexpected number of succeeded pods.  Expected %d, saw %d\n", name, tc.expectedSucceededWorkerPods, actual.Status.ReplicaStatuses[common.ReplicaType(v1beta2.PyTorchReplicaTypeWorker)].Succeeded)
			}
			if actual.Status.ReplicaStatuses[common.ReplicaType(v1beta2.PyTorchReplicaTypeWorker)].Failed != tc.expectedFailedWorkerPods {
				t.Errorf("%s: unexpected number of failed pods.  Expected %d, saw %d\n", name, tc.expectedFailedWorkerPods, actual.Status.ReplicaStatuses[common.ReplicaType(v1beta2.PyTorchReplicaTypeWorker)].Failed)
			}
		}
	...
	testCases := []testCase{
		{
			job: &v1beta2.PyTorchJob{
				...
				Spec: v1beta2.PyTorchJobSpec{
					PyTorchReplicaSpecs: map[v1beta2.PyTorchReplicaType]*common.ReplicaSpec{
						v1beta2.PyTorchReplicaTypeWorker: &common.ReplicaSpec{
