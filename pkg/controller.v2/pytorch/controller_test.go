...
var (
	jobRunning   = v1alpha2.PyTorchJobRunning
	jobSucceeded = v1alpha2.PyTorchJobSucceeded
)
...
func TestNormalPath(t *testing.T) {
	testCases := map[string]struct {
		...
		expectedCondition       *v1alpha2.PyTorchJobConditionType
	...
	for name, tc := range testCases {
		...
		var actual *v1alpha2.PyTorchJob
		...
		for _, controllerRef := range fakePodControl.ControllerRefs {
			if got, want := controllerRef.APIVersion, v1alpha2.SchemeGroupVersion.String(); got != want {
				t.Errorf("controllerRef.APIVersion = %q, want %q", got, want)
			}
			if got, want := controllerRef.Kind, v1alpha2.Kind; got != want {
		...
		if actual.Status.PyTorchReplicaStatuses[v1alpha2.PyTorchReplicaTypeWorker] != nil {
			if actual.Status.PyTorchReplicaStatuses[v1alpha2.PyTorchReplicaTypeWorker].Active != tc.expectedActiveWorkerPods {
				t.Errorf("%s: unexpected number of active pods.  Expected %d, saw %d\n", name, tc.expectedActiveWorkerPods, actual.Status.PyTorchReplicaStatuses[v1alpha2.PyTorchReplicaTypeWorker].Active)
			}
			if actual.Status.PyTorchReplicaStatuses[v1alpha2.PyTorchReplicaTypeWorker].Succeeded != tc.expectedSucceededWorkerPods {
				t.Errorf("%s: unexpected number of succeeded pods.  Expected %d, saw %d\n", name, tc.expectedSucceededWorkerPods, actual.Status.PyTorchReplicaStatuses[v1alpha2.PyTorchReplicaTypeWorker].Succeeded)
			}
			if actual.Status.PyTorchReplicaStatuses[v1alpha2.PyTorchReplicaTypeWorker].Failed != tc.expectedFailedWorkerPods {
				t.Errorf("%s: unexpected number of failed pods.  Expected %d, saw %d\n", name, tc.expectedFailedWorkerPods, actual.Status.PyTorchReplicaStatuses[v1alpha2.PyTorchReplicaTypeWorker].Failed)
			}
		}
