...
type PyTorchJobLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.PyTorchJob, err error)
...
// List lists all PyTorchJobs in the indexer.
func (s *pyTorchJobLister) List(selector labels.Selector) (ret []*v1beta1.PyTorchJob, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.PyTorchJob))
	})
	return ret, err
}
...
type PyTorchJobNamespaceLister interface {
	...
	Get(name string) (*v1beta1.PyTorchJob, error)
...
// List lists all PyTorchJobs in the indexer for a given namespace.
func (s pyTorchJobNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.PyTorchJob, err error) {
...
// Get retrieves the PyTorchJob from the indexer for a given namespace and name.
func (s pyTorchJobNamespaceLister) Get(name string) (*v1beta1.PyTorchJob, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("pytorchjob"), name)
	}
	return obj.(*v1beta1.PyTorchJob), nil
}
