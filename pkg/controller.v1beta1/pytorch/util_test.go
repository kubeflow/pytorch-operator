...
func TestGenOwnerReference(t *testing.T) {
	...
	if ref.APIVersion != v1beta1.SchemeGroupVersion.String() {
		t.Errorf("Expected APIVersion %s, got %s", v1beta1.SchemeGroupVersion.String(), ref.APIVersion)
	}
}
...
func TestGenLabels(t *testing.T) {
	...
	if labels[labelGroupName] != v1beta1.GroupName {
		t.Errorf("Expected %s %s, got %s", labelGroupName, v1beta1.GroupName, labels[labelGroupName])
	}
}
