...
func (pc *PyTorchController) reconcilePods(
	...
	pods []*v1.Pod,
	rtype v1beta1.PyTorchReplicaType,
		...
		if len(podSlice) > 1 {
			...
			if spec.RestartPolicy == common.RestartPolicyExitCode {
				...
				for _, status := range pod.Status.ContainerStatuses {
					...
					if status.Name == v1beta1.DefaultContainerName && state.Terminated != nil {
...
func setClusterSpec(podTemplateSpec *v1.PodTemplateSpec, job *v1beta1.PyTorchJob, totalReplicas int32, index string, rtype v1beta1.PyTorchReplicaType) error {
	rank, err := strconv.Atoi(index)
	if err != nil {
		return err
	}

	masterPort, err := GetPortFromPyTorchJob(job, v1beta1.PyTorchReplicaTypeMaster)
	if err != nil {
		return err
	}

	masterAddr := jobcontroller.GenGeneralName(job.Name, strings.ToLower(string(v1beta1.PyTorchReplicaTypeMaster)), strconv.Itoa(0))
	if rtype == v1beta1.PyTorchReplicaTypeMaster {
		if rank != 0 {
