...
func TestNormalPath(t *testing.T) {
	...
	for name, tc := range testCases {
		...
		for _, controllerRef := range fakePodControl.ControllerRefs {
			if got, want := controllerRef.APIVersion, v1beta1.SchemeGroupVersion.String(); got != want {
				t.Errorf("controllerRef.APIVersion = %q, want %q", got, want)
			}
			if got, want := controllerRef.Kind, v1beta1.Kind; got != want {
		...
		if actual.Status.ReplicaStatuses[common.ReplicaType(v1beta1.PyTorchReplicaTypeWorker)] != nil {
			if actual.Status.ReplicaStatuses[common.ReplicaType(v1beta1.PyTorchReplicaTypeWorker)].Active != tc.expectedActiveWorkerPods {
				t.Errorf("%s: unexpected number of active pods.  Expected %d, saw %d\n", name, tc.expectedActiveWorkerPods, actual.Status.ReplicaStatuses[common.ReplicaType(v1beta1.PyTorchReplicaTypeWorker)].Active)
			}
			if actual.Status.ReplicaStatuses[common.ReplicaType(v1beta1.PyTorchReplicaTypeWorker)].Succeeded != tc.expectedSucceededWorkerPods {
				t.Errorf("%s: unexpected number of succeeded pods.  Expected %d, saw %d\n", name, tc.expectedSucceededWorkerPods, actual.Status.ReplicaStatuses[common.ReplicaType(v1beta1.PyTorchReplicaTypeWorker)].Succeeded)
			}
			if actual.Status.ReplicaStatuses[common.ReplicaType(v1beta1.PyTorchReplicaTypeWorker)].Failed != tc.expectedFailedWorkerPods {
				t.Errorf("%s: unexpected number of failed pods.  Expected %d, saw %d\n", name, tc.expectedFailedWorkerPods, actual.Status.ReplicaStatuses[common.ReplicaType(v1beta1.PyTorchReplicaTypeWorker)].Failed)
			}
		}
