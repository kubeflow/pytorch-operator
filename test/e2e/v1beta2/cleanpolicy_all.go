...
func getReplicaSpec(worker int32) map[v1beta2.PyTorchReplicaType]*common.ReplicaSpec {
	spec := make(map[v1beta2.PyTorchReplicaType]*common.ReplicaSpec)
	spec[v1beta2.PyTorchReplicaTypeMaster] = replicaSpec(1)
	spec[v1beta2.PyTorchReplicaTypeWorker] = replicaSpec(worker)
	return spec
...
func run() (string, error) {
	...
	// Create PyTorchJob
	_, err = torchJobClient.KubeflowV1beta2().PyTorchJobs(*namespace).Create(original)
	if err != nil {
		log.Errorf("Creating the job failed; %v", err)
		return *name, err
	}
	log.Info("Job created: \n%v", util.Pformat(original))
	var torchJob *v1beta2.PyTorchJob
	for endTime := time.Now().Add(*timeout); time.Now().Before(endTime); {
		torchJob, err = torchJobClient.KubeflowV1beta2().PyTorchJobs(*namespace).Get(*name, metav1.GetOptions{})
		if err != nil {
	...
	if err := torchJobClient.KubeflowV1beta2().PyTorchJobs(*namespace).Delete(*name, &metav1.DeleteOptions{}); err != nil {
		log.Fatalf("Failed to delete PyTorchJob %v; error %v", *name, err)
	}

	deleted = false
	for endTime := time.Now().Add(*timeout); time.Now().Before(endTime); {
		_, err = torchJobClient.KubeflowV1beta2().PyTorchJobs(*namespace).Get(*name, metav1.GetOptions{})
